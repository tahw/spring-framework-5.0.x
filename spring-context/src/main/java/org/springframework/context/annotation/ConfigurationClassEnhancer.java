...
class ConfigurationClassEnhancer {
	...
	public Class<?> enhance(Class<?> configClass, @Nullable ClassLoader classLoader) {
		/**
		 * 判断是否已经被代理
		 * 判断是否实现一个接口，这个接口是在生成代理类的时候实现的接口，为beanFactory赋值
		 */
	...
	private Enhancer newEnhancer(Class<?> configSuperClass, @Nullable ClassLoader classLoader) {
		...
		/**
		 * 这里实现BeanFactoryAware接口，非常重要，里面获取到BeanFactory就非常重要
		 * 如果在beanFactory有，直接可以取，如果没有就创建
		 * 配置类就是这样，是生成cglib代理类
		 *
		 * 里面也有描述，如果是一个factoryBean的一个类，在getObject()会又生成一个cglib的代理类，为何？
		 * 因为你在factoryBean里的getObject()对象，他也需要从容器里去取，需要判断
		 */
		...
		/**
		 * 生成的策略
		 * 其中添加属性$$beanFactory去获取实现接口方法里的BeanFactory，其中实现就会变成下列代码
		 * proxy extends AppConfig implements EnhancedConfiguration{
		 ...
		 * }
		 * ......
		 * 大致的内容就是这样，详情见一下实现cglib自己的实现，见spring-cglibself
		 * 其中主要的关键代码enhancer.setCallback(new MyMethodInterceptor())，关键自己去看
		 */
		enhancer.setStrategy(new BeanFactoryAwareGeneratorStrategy(classLoader));
		/**
		 * 这里是cglib去做的拦截，增强方法，处理的东西也是这个去做的
		 * 其中里的callback也是去做一些列事情，其中有主要的是这三个
		 * new BeanMethodInterceptor(),
		 * new BeanFactoryAwareMethodInterceptor(),
		 * NoOp.INSTANCE
		 * 很重要的一个类，BeanMethodInterceptor，这个需要自己去看源码
		 * 调用最主要的方法也是intercept，而这个方法什么时候会触发呢，在需要被代理的类里方法调用的时候
		 */
