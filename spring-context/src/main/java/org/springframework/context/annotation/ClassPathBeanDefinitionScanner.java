...
public class ClassPathBeanDefinitionScanner extends ClassPathScanningCandidateComponentProvider {
	...
	protected Set<BeanDefinitionHolder> doScan(String... basePackages) {
		...
		for (String basePackage : basePackages) {
			/**
			 * 真正操作，将对应的包扫描成对应的bd，spring内部扫描的方式是通过asm方式读取字节码内容，SimpleMetadataReader读取字节码
			 * 用这个字节码实例化了一个ClassReader实例，ClassReader根据JVM规范中class文件的结构，
			 * 分析出了该字节码所表示的类的方法，属性，注解等等所有的类的元信息
			 */
			...
			for (BeanDefinition candidate : candidates) {
				...
				/**
				 * 在findCandidateComponents里实例化bd的类型是ScannedGenericBeanDefinition
				 * ScannedGenericBeanDefinition继承AbstractBeanDefinition
				 */
